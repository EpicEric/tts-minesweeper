...
class ViewController: NSViewController {
    ...
    @IBAction func linesSliderChanged(sender: NSSlider) {
        ...
        minesLabel.stringValue = "Mines: \(mines)"
    ...
    @IBAction func generateButtonPressed(sender: NSButton) {
        ...
        let (created, filename) = json.createFile(lines, collumns: collumns, mines: mines, board: board)
        if (created){
            let alert = NSAlert()
            alert.messageText = "File created at \(filename)!"
            alert.runModal()
        }
    }
